...
#input parameters are:
    ...
    #o -- output folder
############################################
syntax="./Circall.sh -genome [genome in fasta format] -gtfSqlite [genome annotation in Sqlite format] -txFasta [transcripts (cDNA) in fasta format] -txIdx [quasi-index of txFasta] -bsjIdx [quasi-index of BSJ reference fasta file] -read1 [read1 fastq.gz file] -read2 [read2 fastq.gz file] -thread [number of thread] -tag [tag name] -td [TRUE/FALSE value, defaut is TRUE] -c[TRUE/FALSE value, defaut is TRUE] -o [output folder]"
...
if [[ -z "$tandem" ]]; then
   tandem=$(echo "TRUE")
   echo "No specific setting for --tandem, so use the default setting (-td TRUE)"
fi
...
/path/to/linux/bin/Circall_wt -i $IndexTranscriptome -1 <(gunzip -c $inRead1) -2 <(gunzip -c $inRead2) -o $outDirWT_pairedEnd -p $CPUNUM >>$Circall_log 2>&1
...
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>---------------------------------------------
"
/path/to/linux/bin/Circall_bsj -i $IndexBSJ -1 $unmappedFilesMerged1 -2 $unmappedFilesMerged2 -o $outDirBS -p $CPUNUM >>$Circall_log 2>&1
...
/path/to/linux/bin/TxIndexer -t $outFn_getPseudoSeq_fasta -p $CPUNUM -o $outDir_pseudo_Idx -f >>$Circall_log 2>&1

##map paired-end and extract the header of the hits
/path/to/linux/bin/Circall_pseudo -i $outDir_pseudo_Idx -1 $BSJ_read_merged_1 -2 $BSJ_read_merged_2 -o $outDir_pseudo -p $CPUNUM >>$Circall_log 2>&1
##merge
