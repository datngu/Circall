/*
Date:08 Sep 2019
This tool is to map RNA-seq to pseudo sequence of circRNA and/or tandem candidate and extract the header of the hits
...
//  SACollector<IndexT> hitCollector(sidx);
  ...
  string UN_fafn1 = "Mapped_read_" + ftag +".rh";
  ...
  string mapInfofn ="mapInfo_" + ftag +".txt";
  boost::filesystem::path mappedReadfnpath = sfOpts.outputDirectory  / mapInfofn;
  std::unique_ptr<std::FILE, int (*)(std::FILE *)> mapInfoOutput(std::fopen(mappedReadfnpath.c_str(), "a"), std::fclose); 
    ...
    ++hitIndex;
            ...
            if (haveCompat) {
                if (txpIDsCompat.size() > 0) {
                    ...
                    //Circall - export to file
                    // header only - redundant, should be excluded in future
                    fmt::print(UN_faOutput1.get(), header1);
                    fmt::print(UN_faOutput1.get(), "\n");
                    // header + list of txs
                    std::string myoutstr="";
                    for (auto& myid : txpIDsCompat) {
                      myoutstr=myoutstr+transcripts[myid].RefName+" ";
                    }

                    fmt::print(mapInfoOutput.get(), myoutstr);
                    fmt::print(mapInfoOutput.get(), "\n");                    
...
int main(int argc, char* argv[]) {
    ...
    advanced.add_options()
        ...
        ("ignoreLibCompat", po::bool_switch(&(sopt.ignoreLibCompat))->default_value(true), "Enables "
             "strand-aware processing completely.  All hits are considered \"valid\".")//Circall_pseudo default setting
    ...
    try {
        ...
        if ( vm.count("help") ) {
            auto hstring = R"(
                Circall_pseudo
                ==========
                To perform quasi-mapping to pseudo sequences of circRNAs and/or tandem
