/*
Date: 15 Otc 2019
- rename the header of unmapped reads before exporting out
Date: 05 Aug 2019
1) export unmapped reads for paired reads to files: 
- consider individual reads of the pair, a read is mapped if its mapped kmer_len == read_len * 1.0
- consider the unmapped read as read1, the other read of the pair as read2 and export them to file
Date: 18 Sep 2018
- extend unmapped reads for single-end: a mapped read if its mapped kmer_len == read_len * 1.0
Date:16 May 2018
This tool is used to 
1) export unmapped reads (both single-end and paired reads) to files:
# single-end: a mapped read if its mapped kmer_len > read_len * 0.8
# paired-end: if there are no hits --> unmapped reads (not used this moment)
2) export equivalance classes of mapped reads to files
...
//    auto& readBias = readExp.readBias();
  ...
  //these are only for paired-end
  std::vector<uint32_t> txpIDsAll;
  std::vector<double> auxProbsAll;
  //these are also found in single-end
...
/* Process left hits-------------------------------------*/
       /*----------------------------------------------------------------*/
        ...
        bool isKmerMapped{true};
        if (leftFwdSAInts.size()==0 and leftRcSAInts.size()==0){ isKmerMapped=false;}
...
//                if (saIntervalHit.len > (readLen-20)){
                //if (saIntervalHit.len > readLen*0.8){ // less strict for mapped reads
                if (saIntervalHit.len == readLen){
                  isUnmapped=false;
                  break;
                }
            }
...
//            std::cout << " is unmapped?? " << isUnmapped <<std::endl; 
            //Circall - unmapped reads then export
            
            if (isUnmapped and isKmerMapped) {
              //rename header 1 and header 2
              string newheader1 = header1 + "___11";
              string newheader2 = header1 + "___22";

              fmt::print(UN_faOutput1.get(), newheader1);
              fmt::print(UN_faOutput1.get(), "\n");
              fmt::print(UN_faOutput1.get(), j->data[i].first.seq);
              fmt::print(UN_faOutput1.get(), "\n");

              fmt::print(UN_faOutput2.get(), newheader2);
            ...
            upperBoundHits += (leftHits.size() > 0);

            // If the read mapped to > maxReadOccs places, discard it
            if (leftHits.size() > sfOpts.maxReadOccs ) { leftHits.clear(); }

            if (leftHits.size() > 0) {
//            if (leftHits.size() > 0 and isSplit) {
                int32_t fwAll = 0;
                int32_t fwCompat = 0;
                int32_t rcAll = 0;
                int32_t rcCompat = 0;
...
/* Process right hits-------------------------------------*/
       /*----------------------------------------------------------------*/
        // There are 02 kmer lists. Need to check whether their k-mers contain back-splicing junction or not 
        isKmerMapped=true;
        if (rightFwdSAInts.size()==0 and rightRcSAInts.size()==0){ isKmerMapped=false;}
...
//                if (saIntervalHit.len > (readLen-20)){
          ...
          // Case 2: rightRcSAInts that exists hits from forward direction: Detect all cases that hitPos < 100 and hitPos+len>100
          //Check SA intervals
...
//            std::cout << " is unmapped?? " << isUnmapped <<std::endl; 
            ...
            if (isUnmapped and isKmerMapped) {
              string newheader1 = header1 + "___21";
              string newheader2 = header1 + "___12";

                // now read2 is printed to U_R1 and read1 is printed into U_R2
              fmt::print(UN_faOutput1.get(), newheader1);
              fmt::print(UN_faOutput1.get(), "\n");
            ...
            upperBoundHits += (rightHits.size() > 0);

            // If the read mapped to > maxReadOccs places, discard it
            if (rightHits.size() > sfOpts.maxReadOccs ) { rightHits.clear(); }

            if (rightHits.size() > 0) {
//            if (rightHits.size() > 0 and isSplit) {
                int32_t fwAll = 0;
                int32_t fwCompat = 0;
                int32_t rcAll = 0;
                int32_t rcCompat = 0;
...
void processReadsQuasi(single_parser* parser,
  ...
  //generate a unique ID for as single thread
  string ftag=std::to_string(std::rand());
  string UN_fafn1 = "Unmapped_read_" + ftag +".fa";
  boost::filesystem::path UN_readfafn1 = sfOpts.outputDirectory  / UN_fafn1;
  std::unique_ptr<std::FILE, int (*)(std::FILE *)> UN_faOutput1(std::fopen(UN_readfafn1.c_str(), "a"), std::fclose);
    ...
    try {
        ...
        if ( vm.count("help") ) {
            auto hstring = R"(
                Circall_wt
                ==========
                To perform quasi-mapping to wild-type transcriptome to extract unmapped reads
